# Analyse des points, des abonnes et des notifications
...
L'abonnement est aussi le declencheur naturel de notifications: les evenements d'une boutique doivent etre distribues uniquement aux utilisateurs possedant une ligne correspondante dans `shop_follows`.
...
6. Brancher `pages/favorites.js` sur `follow.php` et calculer `followed` par utilisateur dans l'API des boutiques.
7. Supprimer ou deprecier `shops.followed`, qui est un etat global incoherent.
8. Ajouter `notifications`, ses endpoints authentifies et les evenements lies aux boutiques suivies.
9. Corriger la reinitialisation SQL pour supprimer `shop_follows`, `checkins` et les nouvelles tables avant recreation.
10. Ajouter des tests d'integration: session absente, boutique fermee, distance invalide, double check-in, rollback SQL, suivi persiste et notification reservee au bon abonne.

## 6. Resume de la logique metier

Un utilisateur authentifie peut suivre une boutique. Ce suivi est stocke dans `shop_follows`. Lorsqu'il visite cette boutique et passe les controles serveur, un check-in valide peut produire un mouvement de `+10` points et une notification de confirmation. Les futurs achats de recompenses produiront des mouvements negatifs, jamais une modification silencieuse du compteur. Lorsqu'une boutique suivie publie un produit, une offre ou un changement important, chaque abonne recoit une notification persistante, eventuellement relayee en push. Le solde, le suivi et les notifications doivent tous etre decides par le serveur et seulement reflettes par le navigateur.
